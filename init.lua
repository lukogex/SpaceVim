...
local svim_logger = require("spacevim.api.logger"):new({ level = "debug", name = "init" })
...
svim_logger:info('Spacevim started with arguments: ' .. vim.inspect(vim.v.argv))
svim_logger:info('Parsed arguments: ' .. vim.inspect(args))
svim_logger:info('Initial runtimepath: ' .. vim.inspect(vim.opt.rtp:get()))

svim_logger:info('Base directory configuration:')
svim_logger:info('Config directory ($XDG_CONFIG_HOME): ' .. vim.fn.stdpath("config"))
svim_logger:info('Data directory ($XDG_DATA_HOME): ' .. vim.fn.stdpath("data"))
svim_logger:info('Run directory ($XDG_RUNTIME_DIR): ' .. vim.fn.stdpath("run"))
svim_logger:info('State directory ($XDG_STATE_HOME): ' .. vim.fn.stdpath("state"))
svim_logger:info('Cache directory ($XDG_CACHE_HOME): ' .. vim.fn.stdpath("cache"))
svim_logger:info('Log directory: ' .. vim.fn.stdpath("log"))
...
if python_host_prog and python_host_prog ~= '' then
  svim_logger:info('$PYTHON_HOST_PROG is not empty, setting g:python_host_prog:' .. python_host_prog)
...
if python3_host_prog and python3_host_prog ~= '' then
  svim_logger:info('$PYTHON3_HOST_PROG is not empty, setting g:python3_host_prog:' .. python3_host_prog)
