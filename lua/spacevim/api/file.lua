...
function File.override(msg, fname)
  local flags
  if fn.filereadable(fname) == 1 then
    flags = 'b'
  else
...
function File.path_to_fname(path)
  return fn.substitute(File.unify_path(path), '[\\/:;.]', '_', 'g')
end

function File.findfile(what, where, ...)
...
function File.finddir(what, where, ...)
  -- let old_suffixesadd = &suffixesadd
  -- let &suffixesadd = ''
  local count = select(1, ...)
  if count == nil then
